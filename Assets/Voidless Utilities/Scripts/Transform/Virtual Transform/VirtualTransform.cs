...
#region UnityMethods:
	...
	void Update()
	{
		if(!updatingParents) InheritParents();
	}
#endregion
	...
	public void AddParents(params VirtualTransform[] _parents)
	{
		if(_parents != null)
		{
			...
			UpdateLocalProperties(_parents);
		}
	}
	...
	private void UpdateLocalProperties(params VirtualTransform[] _virtualParents)
	{
		updatingParents = true;

		//if(_virtualParents.Length > 0)
		{
			Vector3 accumulatedParentPositions = Vector3.zero;
			Vector3 accumulatedParentEulerRotations = Vector3.zero;
			Vector3 accumulatedParentScales = Vector3.one;
			Quaternion accumulatedRotations = Quaternion.identity;
			float countInverse = virtualParents.Count < 1 ? 1 : (1.0f / (float)virtualParents.Count);
	...
	private void InheritParents()
	{
		if(virtualParents.Count > 0)
		{
			...
			Vector3 accumulatedParentScales = Vector3.one;
			Quaternion accumulatedRotations = Quaternion.identity;
			float countInverse = virtualParents.Count < 1 ? 0 : (1.0f / (float)virtualParents.Count);
			...
			if(inheritProperties.HasFlag(TransformProperties.Rotation)) transform.rotation = Quaternion.Euler((accumulatedParentEulerRotations * countInverse) + localEulerRotation);
